...
| [DEAD-4](#dead-4) | (this pass) | **Deleted rather than wired back in, because the helper was wrong.** `GetAverageMetalBurningTemperature` computed $0.5(T_{melt} - T_s)$ — a *difference*, not a mean — so "wire back in" would have evaluated $\lambda_r$ near 300 K instead of the ~1000 K layer mean it was meant to supply; the surviving §7.1 closure ($k_{rT}$, bounds now `[0,1]`) already covers that role. Removal ran wider than the register's two call sites: the field fed a PDF resource key (`AverageMetalBurningTemperature`, present in all three `.resx` and the generated accessor but **never printed** — no `ProblemContextReport` call site) and an always-zero curve in the `skeleton_layer.json` sidecar and `skeleton_layer_plots.py`. `frame.get(key)` made the Python side tolerant, so dropping only the C# field would have left a silently empty third panel rather than an error. Also renamed the `GetRadiativeThermalConductivity` parameter, still called `averageMetalBurningTemperature` while receiving the radiative-closure temperature. **Not** verified against the standing `--forward-eval` numbers: no `best_vector.txt` exists on `dev`. Justification instead is structural — the field was write-only in the solver (assigned `Zero`, never read by any formula) — plus a green suite, including the parity test that compares every shared $T_s$/$\dot m_d$/$v_b$/flux/height across both tiers. 12 projects / 193 tests and 125 pytest tests green. The same pass also removed `Units/HMetalBurningCoefficient.cs` (an unused 128-line UnitsNet-style quantity for a metal-burning heat-transfer coefficient, W/(K·m²) — zero references outside itself, from the era when $q_{\text{metal}}$ was a convective law rather than the Fourier form of §10). Not in the register; found while tracing the same field. |
...
<a id="new-25"></a>**NEW-25 · Med · S** — The skeleton surface fraction is computed twice, from two different JSON fields.
The model reads `Propellant.PocketSurfaceFractionCoefficients` ([PropellantExtensions.cs:88](../src/dotnet/ParametricCombustionModel/src/ParametricCombustionModel.Computation/Extensions/PropellantExtensions.cs#L88), ascending-power polynomial in p[MPa], divided by `PocketMassFraction`) and feeds it to `PropellantParamsByUnits.SkeletonSurfaceFraction`. The `skeleton_surface_fraction` plot recomputes the same quantity in [main.py:174-180](../src/python/PropellantsPlotRendering/src/main.py#L174) from **`components.Aluminum.agglomeration_coefficients`** instead, over the same `/ pocket_mass_fraction`.

In `data/propellants.01234.json` the two arrays are element-wise identical for all five fuels (verified 2026-07-27), so the plot describes the model today. Nothing enforces that. A regeneration that updates one field and not the other leaves the curve silently describing a different propellant characterisation than the one that was fitted — and the plot is in the PDF, next to the fit it would no longer match. The Python side additionally clamps to `max(0, min(100, ·))` where the C# does not; its docstring records that neither bound binds over the fitted range. → Either read one field in both places, or assert equality where the input JSON is built.

<a id="new-26"></a>**NEW-26 · Low · S** — Cross-composition pressure grids are checked by count, not by value.
`ProblemContextByUnitsMatrixBuilder` takes the pressure grid from `propellants.First()` and then looks every per-pressure frame up with `First(pf => pf.Pressure == pressure.Pascals)` — exact `double` equality. **Within** a group that is safe: a fuel whose grid differs throws loudly. **Across** groups nothing compares them — [GroupOptimizationResultAdapter.cs:60-64](../src/dotnet/ParametricCombustionModel/src/ParametricCombustionModel.Optimization/Results/GroupOptimizationResultAdapter.cs#L60) only verifies that all three compositions have the same `PressureCount`.

So three compositions could carry three different 10-point grids and merge without complaint. [PressureTablesReport.cs:38](../src/dotnet/ParametricCombustionModel/src/ParametricCombustionModel.ReportMaking/Reports/Pdf/PressureTablesReport.cs#L38) then takes each table's pressure heading from `ProblemContextMatrix[0, j]` — row 0, i.e. `Bas_2` — while every cell in the table holds its own fuel's pressure, so all five columns would be filed under `Bas_2`'s pressure. Same defect class as [NEW-23](#new-23), one level up.

Inert on the shipped data: all five fuels' grids are bitwise identical (verified 2026-07-27), and the file is machine-generated from one pressure list. Recorded because the guard that would catch a divergence is the one that is missing. → Compare grid *values* across compositions in the merge, not just counts.

<a id="new-27"></a>**NEW-27 · Med · S — GUARDED, not fixed** — The memetic Nelder–Mead refiner cannot run on `DotNetDifferentialEvolution` 5.x.
`DotNetNelderMead.DifferentialEvolution` 1.0.0 is the only published build of the adapter and is compiled against DE **4.0.0**. Its `NelderMeadLocalSearchRefiner.Refine` calls `ProblemContext.Population` and `.PopulationFfValues`, both **removed in 5.1.0** (superseded by `CurrentPopulation` / `PopulationView`). Confirmed by running it: `MissingMethodException: Method not found: 'System.Memory\`1<Double> DotNetDifferentialEvolution.Models.ProblemContext.get_Population()'`.

The dangerous part is *when* it fails. The assembly links and loads normally — the reference resolves, the build is clean, the search starts — and the fault surfaces only when the refiner first fires, i.e. on the `EveryNGenerations`-th generation. On the production configuration that is hours into a run, with no report written. `NelderMeadRefinementSettings.Validate` therefore rejects `MemeticInLoop` outright, so the failure happens while the plan is being built; pinned by `MemeticInLoopIsRejectedWhileTheAdapterTargetsTheOldLibrary`.

Not a live regression: `MemeticInLoop` is off by default and was measured (2026-06-04) to be a hair *worse* than final-polish-only under jDE, so nothing shipped depends on it. The final polish is unaffected — it goes through `DotNetNelderMead` directly and never touches a DE type. → Publish an adapter built against DE 5.x (both packages are maintained in this project's own upstream), then delete the one `throw`. The v5 replacement seam already exists: `GenerationContext.CurrentPopulation`, plus the documented requirement that a refiner adds its evaluations to `ProblemContext.EvaluationCount`.

<a id="new-23"></a>**NEW-23 · High · S — FIXED in `7473eba`** — The PDF filed the `Bas_2` and `Bas_0` constants under each other's headings.
[GroupCombustionSolverParamsReport.cs:103-129](../src/dotnet/ParametricCombustionModel/src/ParametricCombustionModel.ReportMaking/Reports/Pdf/GroupCombustionSolverParamsReport.cs#L103): the loop passed `comp` (0 = `Bas_2 (includes Bas_3, Bas_4)`, matching `CompositionGroups.ReportNames` and `blockStarts = [11, 18, 25]`), but the value switch read the `…Bas0` properties for `compositionIndex 0` and the `…Bas2` ones for `2` — inverted with respect to [`GroupCombustionSolverParams.ToCompositionVector`](../src/dotnet/ParametricCombustionModel/src/ParametricCombustionModel.Computation/Models/KnownParams/GroupCombustionSolverParams.cs#L124), where `0 → Bas2 = vector[11..17]`. So the seven condensed-phase constants printed under `Bas_2` were `Bas_0`'s and vice versa. `Bas_1` was correct.

Rated High because of *where* it lands: the PDF is the dissertation-facing artefact, and the numbers were all real, all in range, and merely attributed to the wrong fuel — the failure mode that survives review. Confirmed on run E's report (`Bas_2` block showed `A_decompose = 9.99999502723927E14`, which is slot 25). Nothing else is affected: the solve, the fitness, the burn-rate error and Vieille sections (each keyed off `CompositionContexts`) and `best_vector.txt` are all correct, so **every result ever produced stands — only that one PDF section was mislabelled.** Each value was also printed against `blockStarts[comp]`'s bounds rather than its own block's, which happened to render correctly only because all three blocks share the same bounds.

A comment above the loop asserted the inversion was deliberate ("Note: Order is inverted compared to optimizer internals (0=Bas_2, 1=Bas_1, 2=Bas_0)") while naming the mapping that contradicts it. It predates `32dd10b`, which single-sourced the names around it and left the note in place. Same family as [NEW-6](#new-6) and [NEW-22](#new-22) — except here the wrong comment was shielding wrong code, not describing right code.

→ Fixed by aligning the switch with `ToCompositionVector` and replacing the comment. `RailFlagTests` could not have caught it: it drives all 32 genes to one value *by design*, so it is blind to the slot→parameter mapping. Regression coverage added in `CompositionBlockAttributionTests`, which gives every slot a distinct value and asserts each heading prints its own block (verified to fail on the pre-fix code, in exactly the two composition blocks that were swapped).

<a id="new-24"></a>**NEW-24 · Med · S** — The PDF rail flag is meaningless for `A_metal` and `B_metal`.
`RailFlag` scales its tolerance by `max(|bound|, 1)`, so for a bound below 1 the tolerance floors at `1e-3` absolute. `AMetalBurningConstant` is bounded `[1e-10, 1e-3]` and `BMetalBurningConstant` `[1e-12, 1e-5]` — **both boxes are narrower than the floor**, so the lower check always wins and those six lines read `[RAILED@lo]` for every possible value. On run E they do exactly that, including for `B_metal = 1e-5`, which is sitting on its *upper* bound.

The behaviour is already pinned by `DegenerateBoxNarrowerThanTheTolerance_IsRailedLowBecauseTheLowerCheckRunsFirst`, so it is a known property of the heuristic rather than an oversight — but the consequence for these two parameters was not: the flag that exists to tell an operator "widen this bound and re-run" is stuck on for the two parameters whose bounds span seven orders of magnitude and most need it. Recorded rather than fixed because the remedy is a policy choice: judge positive-bounded parameters on a log scale (which is how the campaign's own rail checks have been done by hand), or carry a per-parameter tolerance. Either way `RailFlagTests` needs revisiting in the same change.
...
<a id="new-18"></a>**NEW-18 · Med · —** — Seed control **does** exist, but buys reproducibility only single-threaded. ⚠ **Corrected — the original entry here was wrong.**

The first version of this item claimed the library exposes no seed control at all. That was wrong, and the way it was wrong is the lesson: the check searched for members *named* "Seed", and searched for a `BaseRandomProvider` parameter only on **builder-stage** types. The real seam is neither — it is a constructor parameter on a *strategy*. **Found by the user, not by the tests.** A name-based API search is not an API audit.

What is actually there, verified by reflection and by running it:
- `MutationStrategy` has a public constructor whose last parameter is a `BaseRandomProvider`, and `BaseRandomProvider` is abstract (`Next(int)`, `NextDouble()`), so a seeded subclass drops straight in.
- `IControlParameterProvider.GetControlParameters(int, BaseRandomProvider, out double, out double)` takes the provider **as a parameter** — the adaptive strategies do not own their randomness, they are handed it. So one injected provider determines the mutation draws *and* jDE's F/CR adaptation, which is what makes seeding meaningful rather than partial.
- `WithPopulationSampling(IPopulationSamplingMaker)` fixes the initial population separately.

**The catch, measured rather than assumed.** With seed 42 at `UseProcessors(1)`, three runs returned a bit-identical `1.4602309054630267E-08`, and seed 99 returned something different — so the seed is genuinely consumed. At `UseProcessors(4)`, the same seed 42 gave `1.1968746194676747E-08`, `1.600592364312863E-09`, `5.107157698872387E-09` — **reproducibility is lost as soon as there is more than one worker**, because they draw from the shared provider in whatever order they interleave. The real group run uses `ProcessorCount − 1`.

So the practical conclusion for item 7.1 barely moves, but for a different and better-understood reason: a fixed-seed harness is now *possible*, but only at one worker, which is far too slow for a full campaign. Reproducible full-speed runs need per-worker randomness partitioning, which the library does not offer. A further wrinkle: production reaches jDE via `WithJde(F, CR)`, which bypasses `WithMutationStrategy` and so has no injection point; seeding means reconstructing jDE manually, and `JdeStrategy` takes five parameters `WithJde` fills with non-public defaults, so equivalence is unverified.

Now pinned by four tests replacing the wrong one: the mutation-strategy seam, the externally-supplied adaptive provider, single-worker reproducibility (asserting *both* that the same seed repeats and that a different seed does not, so it cannot pass vacuously), and multi-worker non-reproducibility as an inverted tripwire.

**RESOLVED by the `DotNetDifferentialEvolution` 5.1.0 upgrade (2026-07-29).** The inverted tripwire did its job: 5.x adds `DifferentialEvolutionBuilder.WithSeed(int)` and derives **one generator per worker**, so an individual is built, evaluated and selected end to end by a single worker and nothing depends on interleaving. Measured on a 5-D sphere: seed 42 at 1, 2 and 4 workers each reproduced bit-identically across repeats, and seed 99 differed — reproducible full-speed campaigns are now possible, which is what this item said would need per-worker randomness partitioning the library did not offer. The 4.x seam is dead: the `MutationStrategy` constructor taking a `BaseRandomProvider` is `[Obsolete]` and the injected instance is no longer consulted (the engine supplies providers through `MutationContext`), so code seeded that way keeps compiling and silently stops being seeded — which is precisely how the upgrade was detected, by `SeededSingleWorkerRun_IsReproducible` failing. The four tests are replaced by five written against the new contract, and the seed is now a run-configuration knob (`differentialEvolution.seed`, unseeded by default) recorded in `run_configuration.resolved.json` and the PDF header. The residual caveat is smaller but real and is pinned by `SeededRun_IsNotPortableAcrossWorkerCounts`: individual *i* draws from worker *i mod W*'s stream, so a seed reproduces a run only at the worker count it ran on — and that count is machine-derived, which is why the record carries both numbers.
...
<a id="dead-4"></a>**DEAD-4 · Med · S** — Dead metal-burning-temperature path. **CLOSED** (see resolution log).
`GetAverageMetalBurningTemperature` was implemented twice but never called (both call sites commented out), and `AverageMetalBurningTemperature` was hardcoded to `Temperature.Zero`, so the field in `PocketCombustionParams` was inert. Deleted, together with its unused PDF resource key and its always-zero plot series; the helper's formula was a difference rather than a mean, so wiring it back in as written would have been wrong.
