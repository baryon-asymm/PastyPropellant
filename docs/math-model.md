...
Plain Fourier conduction through the skeleton layer between the melting front $T_{melt}$ and the surface:

$$q_{\text{metal}} \;=\; \lambda_{\text{eff}}\,\frac{T_{melt} - T_s}{\delta_s}$$

> **There is no multiplier on this flux, and that is deliberate.** A contact-area correction $k_c$ dividing it existed as `model.skeletonContactFactor` and was **removed on 2026-09-06**, without a compatibility path. $\delta_s$ enters the model in this equation and nowhere else, so multiplying $\delta_s$, dividing $\lambda_{\text{eff}}$ and dividing $q_{\text{metal}}$ are the same operation as far as any burn-rate observation is concerned: a $k_c$ chosen to improve the fit was algebraically indistinguishable from switching the $\delta_s \le d_{AP}$ constraint (§14) off, while the report kept showing that constraint as satisfied. Every archived configuration carrying the key now fails to load, since the loader rejects the unknown member regardless of its value — including the 25 that set the no-op $k_c = 1$, which are recoverable by deleting the line. The 35 at $k_c = 400$ or $1000$ are runs of a different model and stay unreplayable.

**Code:** [PocketPropellantSolver.cs:626-643](../src/dotnet/ParametricCombustionModel/src/ParametricCombustionModel.Computation/Solvers/PocketPropellantSolver.cs#L626-L643), [PocketPropellantSolver.cs:970-985](../src/dotnet/ParametricCombustionModel/src/ParametricCombustionModel.Computation/Solvers/PocketPropellantSolver.cs#L970-L985).
**Inputs:** $T_{melt}$ from `MetalCombustionParams{,ByUnits}`, supplied by `ModelConstants` through the context builders and configurable as `model.metalMeltingTemperatureKelvins` (default 1300 K); $\delta_s$ (§6.1), $\lambda_{\text{eff}}$ (§7.4).
