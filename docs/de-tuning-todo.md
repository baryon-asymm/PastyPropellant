...
Recommendations for improving the DE configuration used in `RunGroupOptimizationAsync` ([Program.cs:109-159](../src/dotnet/Apps/src/PastyPropellant.ConsoleApp/Program.cs#L109-L159)).
...
**File:** [Program.cs:53-61](../src/dotnet/Apps/src/PastyPropellant.ConsoleApp/Program.cs#L53-L61).
...
**File:** [Program.cs:137](../src/dotnet/Apps/src/PastyPropellant.ConsoleApp/Program.cs#L137).
...
**File:** [Program.cs:111-132](../src/dotnet/Apps/src/PastyPropellant.ConsoleApp/Program.cs#L111-L132).
...
**File:** [Program.cs:151-152](../src/dotnet/Apps/src/PastyPropellant.ConsoleApp/Program.cs#L151-L152).

Current: `F = 0.7`, `CR = 0.9`.
...
**Files:** [Program.cs:153-156](../src/dotnet/Apps/src/PastyPropellant.ConsoleApp/Program.cs#L153-L156); [CustomStagnationStreakTerminationStrategy.cs](../src/dotnet/Apps/src/PastyPropellant.ConsoleApp/CustomStagnationStreakTerminationStrategy.cs); [TimeoutTerminationStrategy.cs](../src/dotnet/Apps/src/PastyPropellant.ConsoleApp/TimeoutTerminationStrategy.cs).
...
| idx | Field | Now LB / UB | Target LB / UB |
|---:|---|---|---|
| 0 | `ADecompose` | `0` / `double.MaxValue` | `1` / `1e9` |
| 1 | `EDecompose` | `0` / `4_422_718` | `5e4` / `3e5` |
| 2, 4, 6 | `AKineticFlame*` | `0` / `1e15` | `1e5` / `1e13` |
| 3, 5, 7 | `EKineticFlame*` | `5e4` / `2e5` | `5e4` / `2.5e5` (or keep) |
| 8, 9, 10 | `Nu*` | `-3.0` / `3.0` | `0.0` / `2.5` |
| 11 | `AMetalBurningConstant` | `1e-15` / `1.0` | `1e-10` / `1e-3` |
| 12 | `BMetalBurningConstant` | `1e-15` / `1.0` | `1e-12` / `1e-5` |
| 13 | `DeltaH` | `-1e12` / `1e12` | `-1e7` / `1e7` |
| 14 | `KDiffusionHeight` | `1e-15` / `1e1` | `1e-3` / `1e1` |
| 15 | `APowOrder` | `1.0` / `1.0` (fixed) | (keep — or remove slot, §1.3) |
| 16 | `BPowOrder` | `2.0` / `2.0` (fixed) | (keep — or remove slot, §1.3) |
| 17 | `KCoefficientRadiationTemperature` | `0.0` / `0.0` (fixed) | (decide — see §1.3) |

### Other knobs (Priorities 2–5)

| Knob | File | Now | Target | Notes |
|---|---|---|---|---|
| `populationSize` | Program.cs:137 | `8·D = 256` | `12·D = 384` | NP must divide processor count — recompute the `for` loop |
| `penaltyRate` (all six) | Program.cs:111 | `1.0` | `0.01` | scale penalties to fitness magnitude |
| `MutationForce` | Program.cs:151 | `0.7` | `0.5` | safer for late phase |
| `CrossoverProbability` | Program.cs:152 | `0.9` | `0.9` (keep) | parameter coupling justifies high CR |
| `TerminationStrategy` | Program.cs:154 | `Timeout(9h)` | `Or(Stagnation(5k, rel 1e-6), Timeout(9h))` | new `OrTerminationStrategy` + relative threshold in `CustomStagnationStreakTerminationStrategy` |
...
| `[17]` KCoefficientRadiationTemperature | Either move to `const = 0` or open bound `[0, 1]` | If const: D 16 → 15, 30 → 29 |
