...
| Skeleton surface fraction `f_s` closure | `Computation/Models/KnownParams/SkeletonSurfaceFractionSettings.cs` (`Polynomial` — the historical per-propellant fit) | `model.skeletonSurfaceFraction.mode`; `EquilibriumCarbon` instead computes `f_s = n_C(s)(T_pore, p) / n_C,total` from `model.skeletonSurfaceFraction.equilibriumTableFile`. See *Skeleton coverage* below |
...
Two closures exist. `Polynomial` (default) is the historical per-propellant fit shipped in `propellants*.json`: 22 coefficients across five compositions, byte-identical to `agglomeration_coefficients`, none transferable to an unmeasured composition, four going negative or turning upward outside the fitted 1–6.5 MPa window.
