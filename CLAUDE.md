...
| Model constant — metal melting temperature | `Computation/Models/KnownParams/ModelConstants.cs` (1300 K) | `model.metalMeltingTemperatureKelvins`. Not fitted and not per-propellant, but it changes every computed number, which is why it is configuration: a 2300 K campaign used to need a source patch and left no trace in any report |
...
⚠ **The skeleton contact factor is gone.** `model.skeletonContactFactor` — a divisor on the metal heat flux, default 1.0 — was **removed on 2026-09-06 with no backward compatibility**: the metal flux is now plain Fourier conduction, and a config file carrying that key throws at startup as an unknown member. The reason it had to go rather than be frozen: δ (the skeleton-layer thickness) enters the model only through the flux that factor divided, so any value other than 1 was algebraically the same as switching the δ ≤ d_AP constraint off, while the constraint still printed as satisfied. Consequence to know before replaying anything: **all 60 archived configs under `docs/results/` that carry the key fail to load** — the loader rejects the unknown member itself, so the 25 that merely set it to the no-op 1.0 die exactly like the 35 of the k_c campaign (27 at 1000, 8 at 400). The 1.0 archives are recoverable by deleting the line; the k_c ones are runs of a different model and stay unreplayable by design.
