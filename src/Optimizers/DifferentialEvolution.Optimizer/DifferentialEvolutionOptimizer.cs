...
public class DifferentialEvolutionOptimizer
{
    ...
    public DifferentialEvolutionOptimizer(IMutationStrategy mutationStrategy,
    ...
    private void RunWorker(int workerIndex)
    {
        ...
        while (sum < _numberOfGenerations)
        {
            for (var j = workerIndex; j < _currentPopulation.Individuals.Length; j += _threadNumber)
            {
                ...
                tempIndividual.FitnessFunctionCost = _fitnessFunctionInvoker.Invoke(tempIndividual.Vector.Span);
            ...
            else
            {
                ...
                if (Math.Abs(_currentPopulation.Individuals.Span[lastBestIndividualIndex].FitnessFunctionCost -
                             _currentPopulation.Individuals.Span[_indexOfBestIndividual].FitnessFunctionCost) < 1e-12
                    && _currentPopulation.Individuals.Span[_indexOfBestIndividual].FitnessFunctionCost != double.MaxValue)
                    sum++;
                else
                {
