...
public:
    differential_evolution_provider(const std::function<double(const std::vector<double>&)>& objective_function,
                                    ...
                                    bool minimize,
                                    size_t maxGenerations = 2000,
                                    double weight = 0.5,
                                    double crossover = 0.9)
    {
        amichel::de::processor_listener_ptr processor_listener = std::make_shared<amichel::de::null_processor_listener>();
        ...
        for (size_t i = 0; i < constraints.size(); ++i)
            de_constraints->operator[](i) = std::make_shared<amichel::de::real_constraint>(constraints[i]->get_min(), constraints[i]->get_max());

        termination_strategy = std::make_unique<amichel::de::termination_strategy>(amichel::de::max_gen_termination_strategy { maxGenerations });

        selection_strategy = std::make_unique<amichel::de::selection_strategy>(amichel::de::best_parent_child_selection_strategy {});

        amichel::de::mutation_strategy_arguments mutation_arguments(weight, crossover);
        amichel::de::mutation_strategy_ptr mutation_strategy(std::make_shared<amichel::de::mutation_strategy_1>(constraints.size(), mutation_arguments));
