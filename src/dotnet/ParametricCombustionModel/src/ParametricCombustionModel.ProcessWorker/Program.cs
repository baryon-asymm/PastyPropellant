...
EventBus<InfoLogEvent>.Subscribe(logEvent => {
    Console.WriteLine($"{DateTime.Now.ToShortTimeString()} | {logEvent.Sender ?? "none"} | {logEvent.Message}");
});
...
{
    EventBus<InfoLogEvent>.Publish(new InfoLogEvent(operationResult.Exception.ToString(), nameof(Program)));
}
